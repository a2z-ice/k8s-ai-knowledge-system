...
5. Publish (activate) using the IDs from step 4. If IDs match the known values, use:
   ```bash
   kubectl --context kind-k8s-ai-classic -n k8s-classic-ai exec ${N8N_POD} -- n8n publish:workflow --id=sLFyTfSNzFIiVC9t
   kubectl --context kind-k8s-ai-classic -n k8s-classic-ai exec ${N8N_POD} -- n8n publish:workflow --id=5cf0evFgopkFXM7q
   kubectl --context kind-k8s-ai-classic -n k8s-classic-ai exec ${N8N_POD} -- n8n publish:workflow --id=JItVx5wVu0WTIvkA
   ```
