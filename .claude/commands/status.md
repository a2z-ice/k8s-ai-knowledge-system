...
5. **kind cluster** — `kubectl --context kind-k8s-ai-classic get nodes` — verify cluster is reachable
6. **n8n workflows (active check)** — Test the webhook endpoints directly, as `n8n list:workflow` does not expose the active flag:
   - `curl -s -o /dev/null -w "%{http_code}" http://localhost:31000/webhook/k8s-ai-chat/chat` → 200 means AI workflow is active
   - `curl -s -o /dev/null -w "%{http_code}" -X POST http://localhost:31000/webhook/k8s-reset -H 'Content-Type: application/json' -d '{}'` → 200 means Reset workflow is active
...
- **Pods not Running** → `kubectl --context kind-k8s-ai-classic apply -f infra/k8s/` (already done in Step 0)
- **Qdrant collection missing** → `curl -X PUT http://localhost:31001/collections/k8s -H 'Content-Type: application/json' -d @infra/schemas/qdrant_k8s_collection_schema.json`
- **Qdrant points_count = 0** → `curl -X POST http://localhost:31000/webhook/k8s-reset` then wait 45s
...
- **k8s-watcher image not found** → `docker build -t k8s-watcher-classic:latest ./k8s-watcher/ && kind load docker-image k8s-watcher-classic:latest --name k8s-ai-classic`
